...
const populateList = async() => {
  ...
  data.forEach((item) => {
    ...
    string += `<li class="todo-element" data-priorty="${item.priority}" data-id="${item.id}" data-completed="${item.is_completed}">
        <div onclick="req(${item.id}, '${item.task}', '${item.priority}', ${item.is_completed == 0 ? 1 : 0}, 'UPDATE')"  class="circle" style="background: ${item.is_completed == 0 ? '#444444' : '#bac8ff'}; cursor: pointer; padding: 0.5rem 0.5rem; display: inline; border-radius:100px;"></div>
